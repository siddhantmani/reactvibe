"use client"

import LinearReveal from '@/components/LinearReveal'
import { Bricolage_Grotesque } from 'next/font/google';
import React from 'react'
import GlideAccordionItemsPreview from './GlideAccordionItemsPreview';
import { motion } from 'framer-motion'
import TablePorpsForGlideAccordionItems from './TablePorpsForGlideAccordionItems';

const LinearRevealFont = Bricolage_Grotesque({
    weight: '700',
    subsets: ['vietnamese']
});

function GlideAccordionItems() {
    return (
        <div className='px-2 md:px-5 lg:px-8 xl:px-10 xl:max-w-5xl 2xl:max-w-7xl lg:max-w-3xl mx-auto space-y-7'>

            <div className='space-y-3'>
                <LinearReveal
                    as={'h1'}
                    className={`${LinearRevealFont.className} text-4xl md:text-5xl`}
                    Text='Glide Accordion'
                />

                <motion.p
                    initial={{ opacity: 0, filter: "blur(8px)" }}
                    whileInView={{ opacity: 1, filter: "blur(0px)" }}
                    viewport={{
                        once: true,
                        amount: 0.2, // Trigger when 20% visible
                        margin: "50px"
                    }}
                    transition={{
                        duration: 1.2,
                        ease: [0.25, 0.46, 0.45, 0.94], // Custom easing
                        delay: 0.2
                    }}
                    exit={{ opacity: 0, y: -10 }}
                    className="text-black/70 dark:text-white/70 text-[13px]">
                    A clean and minimal React FAQ accordion component designed for modern landing pages and documentation layouts. This responsive accordion UI uses a structured, easy-to-scan layout to present frequently asked questions with smooth expand and collapse interactions. Ideal for developers building a React FAQ section component with a focus on clarity, usability, and scalable UI design.
                </motion.p>
            </div>

            <div className='border-black rounded-2xl'>
                <GlideAccordionItemsPreview />
            </div>

            <div>
                <TablePorpsForGlideAccordionItems />
            </div>
        </div>
    )
}

export default GlideAccordionItems