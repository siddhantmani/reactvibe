...
import BudgetsMetricsGrid from "./BudgetsMetricsGrid";
import BudgetOverview from "./BudgetOverview";
import BudgetvsActual from "./BudgetvsActual";
import BudgetCategoriesTable from "./BudgetCategoriesTable";
import UpcomingBudgetAlerts from "./UpcomingBudgetAlerts";
    ...
    return (
        <div className="w-64 h-full bg-[#F9FAF6] backdrop-blur-4xl relative p-3 px-3 rounded-tl-[15px] rounded-bl-4xl overflow-hidden">
            ...
            <nav className="space-y-2 pt-2">
                {items.map((item, i) => (
                    <div key={i}
                        className={`px-3 py-2 rounded-lg flex items-center gap-2 text-sm cursor-pointer ${item.name === "Budgets"
...
function BudgetsDashboard() {
    return (
        <div className='flex justify-center'>
            ...
            <div className=" bg-[#f8f8f8] border border-black/5 px-2 py-2 rounded-br-2xl rounded-tr-2xl space-y-2 max-w-7xl">
                <BudgetsHeader />
                <BudgetsMetricsGrid />
