...
export default function OverlayMenubar() {
    ...
    return (
        <div className=" dark:bg-black bg-white overscroll-none">
            {/* Top Bar */}
            <div className={`flex justify-between px-5 p-3 z-50  ${open ? 'hidden' : ''}`}>
                <h1 className={`${CTAButton.className} text-2xl text-black dark:text-white`}>React Vibe</h1>
            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="flex flex-col h-screen w-full justify-between z-50 p-5 bg-white dark:bg-black"
                    >
                        <div className="flex-1 text-white dark:bg-black">
                            {/* Header */}
                                    ...
                                    >
                                        <div aria-label={item.subMenu_key}>
                                            <Link href={item.link}>
                                                <LinearReveal
                                                    Text={item.subMenu}
                                                    className="text-[35px] dark:text-white text-black"
                                                />
                                            </Link>
                                        </div>
                                    </motion.div>
                        ...
                        {/* Social icons */}
                        <div className="w-full pb-4 dark:bg-black text-white">
