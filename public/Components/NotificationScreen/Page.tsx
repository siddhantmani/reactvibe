...
function page() {
    ...
    return (
        <div className='pt-15 overflow-hidden bg-white dark:bg-[#0c0c0c] rounded-2xl md:-mt-6 flex flex-col justify-center items-center'>
            <div className='border-[8px] border-[#131925] dark:border-[#2a2e36] p-4 rounded-[50px] bg-[#fbfbfb] dark:bg-[#0c0c0c] max-w-xs h-fit'>
                {/* Status Bar */}
                <div className='flex justify-between items-center mb-6'>
                    <p className='text-xs font-medium dark:text-white'>{statusBarTime}</p>
                    <div className='bg-[#131925] dark:bg-[#2a2e36] rounded-full p-3 py-3 w-full max-w-[90px]' />
                    <div className='flex items-center gap-1 text-sm'>
                        <Signal size={14} className='dark:text-white' />
                        <Wifi size={14} className='dark:text-white' />
                        <Battery size={14} className='fill-black dark:text-white' />
                    </div>
                </div>

                {/* Time Display */}
                <div className='flex flex-col justify-center items-center text-base/30 py-4 mb-4'>
                    <p className=' text-sm opacity-50 font-semibold dark:text-white/70'>{date}</p>
                    <h1 className='text-[55px] opacity-90 font-extrabold leading-none dark:text-white'>{displayTime}</h1>
                </div>
